...
public final class ThreadStorageTest {
  ...
  public void detach_threadLocalClearedOnRoot() {
    ...
    assertThat(DefaultThreadStorage.localContext.get()).isSameInstanceAs(context);
    storage.detach(context, old);
    // thread local must contain null to avoid leaking our ClassLoader via ROOT
    assertThat(DefaultThreadStorage.localContext.get()).isNull();
  }
  ...
  public void detach_detachRoot() {
    ...
    Logger logger = Logger.getLogger(ThreadStorage.class.getName());
